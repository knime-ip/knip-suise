...
public class LabelingCompareNodeModel<L extends Comparable<L>> extends
    ...
    protected BufferedDataTable[] execute(BufferedDataTable[] inData,
        ...
        int colIdx1 =
                NodeUtils.autoColumnSelection(inData[0].getDataTableSpec(),
                        m_labelingCol1, LabelingValue.class, this.getClass());
        int colIdx2 =
                NodeUtils.autoColumnSelection(inData[0].getDataTableSpec(),
                        m_labelingCol2, LabelingValue.class, this.getClass());
    ...
    private CellFactory createCellFactory(final int colIdx1, final int colIdx2,
        ...
        final ImgPlusCellFactory cellFac =
                exec != null ? new ImgPlusCellFactory(exec) : null;
        ...
        return new CellFactory() {
            ...
            public DataColumnSpec[] getColumnSpecs() {
                DataColumnSpec[] specs =
                        new DataColumnSpec[featFac.getNumFeatures() + 1];

                specs[0] =
                        new DataColumnSpecCreator("Difference Image",
                                ImgPlusCell.TYPE).createSpec();
                for (int i = 1; i < specs.length; i++) {
                    specs[i] =
                            new DataColumnSpecCreator(
                                    featFac.getFeatureNames()[i - 1],
            ...
            public DataCell[] getCells(DataRow row) {
                ...
                Labeling<L>[] labs = new Labeling[2];
                labs[0] =
                        ((LabelingValue<L>)row.getCell(colIdx1)).getLabeling();
                if (row.getCell(colIdx2).isMissing()) {
                    ...
                    labs[1] = (Labeling<L>)labs[0].factory().create(labs[0]);
                } else {
                    labs[1] =
                            ((LabelingValue<L>)row.getCell(colIdx2))
                                    .getLabeling();
                }
                ...
                for (FeatureSet fset : featureSets) {
                    if (fset instanceof LabelingCompareFeatureSet) {
                        Img<ByteType> img =
                                ((LabelingCompareFeatureSet)fset).getDiffImg();
                        try {
                            cells[0] =
                                    cellFac.createCell(new ImgPlus<ByteType>(
                                            img));
                        } catch (IOException e) {
