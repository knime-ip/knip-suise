...
public class GaborFilterFeatureSet<T extends RealType<T>, F extends RealType<F>>
     ...
     * @param averageAngles averages over the given number of angles, thus,
    ...
    private void init() {
        ...
        int numFilters =
                m_halfNumAng * m_scales.length * m_frequencies.length
        ...
        UnaryOperationAssignment<ComplexDoubleType, DoubleType> realAssignment =
                new UnaryOperationAssignment(new ComplexRealToRealAdapter());

        UnaryOperationAssignment<ComplexDoubleType, DoubleType> imaginaryAssignment =
                new UnaryOperationAssignment(
            ...
            for (int s = 0; s < m_scales.length; s++) {
                for (int f = 0; f < m_frequencies.length; f++) {
                    for (int e = 0; e < m_elongations.length; e++) {
                        Gabor g =
                                new Gabor(m_radius, ang, m_scales[s],
    ...
    public double value(int id) {
        ...
        if (m_precalcFeatures) {
            ...
            // - m_filterSizeY / 2);
            RandomAccess<F> tmpFeatRandAccess =
                    m_convolvedImgRandAccess[m_currentAngID % m_halfNumAng
                            * m_numFeatures + id];
            ...
            res =
                    convolve(m_tmpSrcRandAccess, m_filterCursors[m_currentAngID
                            % m_halfNumAng * m_numFeatures + id], m_pos,
                            m_kernelRadii);
    ...
    private void fftConvolveImage() {
        ...
        Convolver<T, DoubleType, F> ics =
                new ImgLib2FourierConvolver<T, DoubleType, F>();

        for (int i = 0; i < m_filters.length; i++) {
            try {
                m_convolvedImgs[i] =
                        m_img.factory().imgFactory(m_featureType)
    ...
    public void showFilterBank() {
        ...
        for (int i = 0; i < m_filters.length; i++) {
            ...
            String name =
                    m_names[i % (m_filters.length / m_halfNumAng)] + "angIdx:"
                            + angIdx + ";ang:" + ang;

            Img<FloatType> floatImg =
                    ImgUtils.createEmptyCopy(img, new FloatType());
            ...
            Pair<FloatType, FloatType> minmax =
                    Operations.compute(new MinMax<FloatType>(), floatImg);
    ...
    public static <T extends RealType<T>> void main(final String[] args) {
        ...
        GaborFilterFeatureSet<DoubleType, FloatType> gfb =
                new GaborFilterFeatureSet<DoubleType, FloatType>(new double[]{
                        .25f, .5f}, new double[]{1}, new double[]{1}, 20, 12,
                        true, new FloatType());
    ...
    public void generateFeatImg(RandomAccessibleInterval<F> featImg,
        ...
        Convolver<T, DoubleType, F> ics =
                new ImgLib2FourierConvolver<T, DoubleType, F>();
        Img<DoubleType> filter =
                m_filters[angleID % m_halfNumAng * m_numFeatures + featID];
     ...
     * 
     * @param img the image in the spatial domain
     * 
     * @param kerC the kernel in the spatial domain
     * 
     * @param pos the position to apply the kernel
     * 
     * @return
     */
