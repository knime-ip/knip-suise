...
public class BoundaryModelNodeModel<F extends RealType<F>, T extends RealType<T>, L extends Comparable<L>>
    ...
    private SettingsModelString m_selectionStrategy =
            createSelectionStrategyModel();

    private SettingsModelWekaClassifier m_classifier = createClassifierModel();

    private SettingsModelString m_optionalParameters =
            createOptionalParametersModel();

    private SettingsModelString m_orientationDimLabel =
            createOrientationDimLabelModel();

    private SettingsModelString m_featureDimLabel =
    ...
    protected PortObject[] execute(PortObject[] inObjects, ExecutionContext exec)
        ...
        HashMap<String, String> map =
                new HashMap<String, String>(splits.length);
        for (String s : splits) {
        ...
        while (rowIterator.hasNext()) {
            ...
            ImgPlus<F> img =
                    ((ImgPlusValue<F>)row.getCell(firstColIdx)).getImgPlus();
            Img<T> srcImg = null;
            if (srcImgColIndex > -1) {
                srcImg =
                        ((ImgPlusValue<T>)row.getCell(srcImgColIndex))
                                .getImgPlus();
            ...
            int dimFeat =
                    img.dimensionIndex(Axes.get(m_featureDimLabel
                            .getStringValue()));
            int dimAngle =
                    img.dimensionIndex(Axes.get(m_orientationDimLabel
                            .getStringValue()));
            ...
            BufferedPixelFeatureSet<F> featSet =
                    new BufferedPixelFeatureSet<F>(
                            new String[(int)img.dimension(dimFeat)], DIM_X,
                            DIM_Y, dimFeat, dimAngle);
            m_boundaryModel.setFeatureSet(featSet);

            LabelingValue<L> labVal =
                    (LabelingValue<L>)row.getCell(secondColidx);
        ...
        if (inObjects[1] == null) {
            ...
            int maskImgColIndex =
                    getMaskImgColIndex(inTable1.getDataTableSpec());
            ...
            while (rowIt.hasNext()) {
                ...
                if (maskImgColIndex != -1) {
                    mask =
                            ((ImgPlusValue<BitType>)row
                                    .getCell(maskImgColIndex)).getImgPlus();
                }
                DefaultRow res =
                        new DefaultRow(row.getKey(), computeResult(
    ...
    private int getSrcImgColIndex(DataTableSpec inSpec)
        ...
        if (m_srcImgCol.getStringValue() != null) {
            colIdx =
                    NodeUtils.autoColumnSelection(inSpec, m_srcImgCol,
    ...
    private int getApplyImgColIndex(DataTableSpec inSpec)
        ...
        if (m_applyImgCol.getStringValue() != null) {
            colIdx =
                    NodeUtils.autoColumnSelection(inSpec, m_applyImgCol,
                            ImgPlusValue.class, this.getClass());
        }
        return colIdx;
    }
    ...
    private int getMaskImgColIndex(DataTableSpec inSpec)
        ...
        if (m_maskImgCol.getStringValue() != null) {
            colIdx =
                    NodeUtils.autoColumnSelection(inSpec, m_maskImgCol,
                            ImgPlusValue.class, this.getClass());
        }
        return colIdx;
    }
    ...
    private Collection<Polygon> getPolygons(Labeling<L> lab) {
        ...
        for (L label : lab.getLabels()) {
            Img<BitType> mask =
                    binaryMask(lab.getIterableRegionOfInterest(label)
    ...
    private ListCell computeResult(ImgPlusValue<F> val, Img<BitType> mask)
        ...
        int dimFeat =
                img.dimensionIndex(Axes.get(m_featureDimLabel.getStringValue()));
        int dimAngle =
                img.dimensionIndex(Axes.get(m_orientationDimLabel
                        .getStringValue()));
        if (dimFeat == -1 || dimAngle == -1) {
        ...
        if (mask == null) {
            mask =
                    ...
                    new ConstantImg<BitType>(new long[]{img.dimension(0),
                            img.dimension(1)}, new BitType(true));
        }
        Img<ByteType>[] classResultImg =
                m_boundaryModel.classifyImageContourModelwise(new int[2], mask);

        // create cells
        ArrayList<ImgPlusCell> cells =
                new ArrayList<ImgPlusCell>(classResultImg.length);
        for (int i = 0; i < classResultImg.length; i++) {
            ImgPlusCell<ByteType> cell =
                    m_imgCellFactory.createCell(
                            classResultImg[i],
